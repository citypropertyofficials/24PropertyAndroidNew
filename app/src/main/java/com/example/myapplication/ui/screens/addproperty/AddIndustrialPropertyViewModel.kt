...
class AddIndustrialPropertyViewModel(
    ...
    private val _uiState = MutableStateFlow(
        AddIndustrialPropertyUiState(
            propertyId = propertyId,
            isEditMode = !propertyId.isNullOrBlank(),
            isInitialLoading = !propertyId.isNullOrBlank()
        )
    )
    val uiState: StateFlow<AddIndustrialPropertyUiState> = _uiState.asStateFlow()

    private val _events = MutableSharedFlow<AddIndustrialPropertyEvent>()
    val events: SharedFlow<AddIndustrialPropertyEvent> = _events.asSharedFlow()
    ...
    fun setImages(images: List<Uri>) {
        ...
        if (deduped.size < _uiState.value.imageUris.size + images.size) {
            viewModelScope.launch {
                _events.emit(AddIndustrialPropertyEvent.ShowMessage("Maximum $MAX_PROPERTY_IMAGES images allowed."))
            }
        }
    }
    ...
    private fun submit(contentResolver: ContentResolver, isDraft: Boolean) {
        ...
        if (errors.isNotEmpty()) {
            ...
            viewModelScope.launch {
                _events.emit(AddIndustrialPropertyEvent.ShowMessage("Please complete the required fields before saving."))
            }
            return
        }
        ...
        viewModelScope.launch {
            try {
                ...
                val draftPayload = buildIndustrialPropertyPayload(
                    state = currentState,
                    userId = userId,
                ...
                val payload = buildIndustrialPropertyPayload(
                    state = currentState,
                    userId = userId,
                    ownerName = userData.name.ifBlank { "Property Owner" },
                    ownerEmail = userData.email,
                ...
                _events.emit(
                    AddIndustrialPropertyEvent.ShowMessage(
                        when {
                            currentState.isEditMode && isDraft -> "Industrial property saved as draft."
                            currentState.isEditMode -> "Industrial property updated successfully."
                            isDraft -> "Industrial property saved as draft."
                            else -> "Industrial property added successfully."
                        }
                    )
                )
                _events.emit(AddIndustrialPropertyEvent.PropertySaved)
                _uiState.value = AddIndustrialPropertyUiState()
            } catch (t: Throwable) {
                _uiState.value = _uiState.value.copy(isSavingDraft = false, isPublishing = false)
                _events.emit(AddIndustrialPropertyEvent.ShowMessage(t.message ?: "Unable to save property."))
            }
        }
    }

    private fun validate(state: AddIndustrialPropertyUiState): Map<String, String> {
        ...
        viewModelScope.launch {
            try {
                ...
                _uiState.value = AddIndustrialPropertyUiState(
                    ...
                    fieldValues = property.baseFieldValues(defaultIndustrialFieldValues()),
                ...
                _events.emit(AddIndustrialPropertyEvent.ShowMessage(t.message ?: "Unable to load property."))
            }
        }
    }
}
