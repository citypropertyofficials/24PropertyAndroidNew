...
class AddResidentialPropertyViewModel(
    ...
    private val _uiState = MutableStateFlow(
        AddResidentialPropertyUiState(
            propertyId = propertyId,
            isEditMode = !propertyId.isNullOrBlank(),
            isInitialLoading = !propertyId.isNullOrBlank()
        )
    )
    val uiState: StateFlow<AddResidentialPropertyUiState> = _uiState.asStateFlow()

    private val _events = MutableSharedFlow<AddResidentialPropertyEvent>()
    val events: SharedFlow<AddResidentialPropertyEvent> = _events.asSharedFlow()
    ...
    fun setImages(images: List<Uri>) {
        ...
        if (deduped.size < _uiState.value.imageUris.size + images.size) {
            viewModelScope.launch {
                _events.emit(AddResidentialPropertyEvent.ShowMessage("Maximum $MAX_PROPERTY_IMAGES images allowed."))
            }
        }
    }
    ...
    private fun submit(contentResolver: ContentResolver, isDraft: Boolean) {
        ...
        viewModelScope.launch {
            try {
                ...
                val draftPayload = buildResidentialPropertyPayload(
                    state = currentState,
                    userId = userId,
                ...
                val resolvedPropertyId = currentState.propertyId
                    ?: addPropertyRepository.createProperty(draftPayload)
                ...
                val payload = buildResidentialPropertyPayload(
                    state = currentState,
                    userId = userId,
                    ownerName = userData.name.ifBlank { "Property Owner" },
                    ownerEmail = userData.email,
                ...
                _events.emit(
                    AddResidentialPropertyEvent.ShowMessage(
                        when {
                            currentState.isEditMode && isDraft -> "Residential property saved as draft."
                            currentState.isEditMode -> "Residential property updated successfully."
                            isDraft -> "Residential property saved as draft."
                            else -> "Residential property added successfully."
                        }
                    )
                )
                _events.emit(AddResidentialPropertyEvent.PropertySaved)
                _uiState.value = AddResidentialPropertyUiState()
            } catch (t: Throwable) {
                _uiState.value = _uiState.value.copy(isSavingDraft = false, isPublishing = false)
                _events.emit(AddResidentialPropertyEvent.ShowMessage(t.message ?: "Unable to save property."))
            }
        }
    }

    private fun validate(state: AddResidentialPropertyUiState): Map<String, String> {
    ...
    private fun loadProperty(propertyId: String) {
        viewModelScope.launch {
            try {
                ...
                val fieldValues = property.baseFieldValues(defaultResidentialFieldValues()).apply {
                ...
                _events.emit(AddResidentialPropertyEvent.ShowMessage(t.message ?: "Unable to load property."))
            }
        }
    }
}
