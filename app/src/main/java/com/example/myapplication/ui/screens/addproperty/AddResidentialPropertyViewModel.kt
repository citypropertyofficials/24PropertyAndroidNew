...
class AddResidentialPropertyViewModel(
    ...
    private val _uiState = MutableStateFlow(AddResidentialPropertyUiState())
            ...
            try {
                ...
                _events.emit(
                    AddResidentialPropertyEvent.ShowMessage(
                        if (isDraft) "Residential property saved as draft." else "Residential property added successfully."
                    )
                )
    ...
    private fun buildPropertyPayload(
        ...
        dynamicValues[FirebaseConstants.FIELD_POSSESSION_STATUS] =
            normalizePossessionStatus(values["possessionStatus"].orEmpty())
